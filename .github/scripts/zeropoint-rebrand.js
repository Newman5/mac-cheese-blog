/**
 * mac-cheese-blog Template Rebranding Script
...
async function rebrand() {
  ...
  // Safety check - ensure we're working with a mac-cheese-blog template
  const packageJson = JSON.parse(readFileSync('package.json', 'utf8'));
  if (packageJson.name !== 'mac-cheese-blog') {
    // Clean up template marker if this isn't a mac-cheese-blog template
    unlinkSync('.template');
    if (isCI) {
      console.log('⚠️ Not a mac-cheese-blog template, skipping...');
    }
    process.exit(0);
  }

  console.log('🪐 Welcome to mac-cheese-blog! Let\'s customize this for your project.\n');
  ...
  replaceInFiles('mac-cheese-blog', projectName.replace(/\s+/g, '')); // Remove spaces from project name
  replaceInFiles('copilot-swe-agent[bot]', author); // Replace default author
  replaceInFiles('mac-cheese-blog', projectName.toLowerCase().replace(/\s+/g, '-')); // Lowercase with hyphens
  ...
  if (existsSync('README.mac-cheese-blog.md')) {
    try {
      copyFileSync('README.mac-cheese-blog.md', 'README.md'); // Use mac-cheese-blog README as new README
    } catch (error) {
      console.warn(`Could not copy README.mac-cheese-blog.md: ${error.message}`);
    }
  }
